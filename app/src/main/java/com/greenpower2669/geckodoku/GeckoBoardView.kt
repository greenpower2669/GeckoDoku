...
class GeckoBoardView @JvmOverloads constructor(
    ...
    private val gestures = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onLongPress(e: MotionEvent) {
            ...
            if (cell != null) onLongPressCell?.invoke(cell) else onLongPressOutside?.invoke()
        }
    })
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(w, resolveSize(w + gutter.toInt(), heightMeasureSpec))
    }
    ...
    private fun drawCross(canvas: Canvas, rect: RectF, alpha: Float) {
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = cellSize * .075f
        paint.strokeCap = Paint.Cap.ROUND
        paint.color = Color.argb((255 * alpha).toInt(), 45, 45, 45)
        val m = cellSize * .29f
        canvas.drawLine(rect.left + m, rect.top + m, rect.right - m, rect.bottom - m, paint)
        canvas.drawLine(rect.right - m, rect.top + m, rect.left + m, rect.bottom - m, paint)
    }

    private fun drawGecko(canvas: Canvas, rect: RectF, alpha: Float, alert: Boolean) {
        val cx = rect.centerX()
        val cy = rect.centerY()
        ...
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = cellSize * .055f
        paint.strokeCap = Paint.Cap.ROUND
        val leg = cellSize * .20f
        canvas.drawLine(cx-cellSize*.08f, cy-cellSize*.05f, cx-leg, cy-cellSize*.15f, paint)
        canvas.drawLine(cx+cellSize*.08f, cy-cellSize*.05f, cx+leg, cy-cellSize*.15f, paint)
        canvas.drawLine(cx-cellSize*.08f, cy+cellSize*.10f, cx-leg, cy+cellSize*.20f, paint)
        canvas.drawLine(cx+cellSize*.08f, cy+cellSize*.10f, cx+leg, cy+cellSize*.20f, paint)
        ...
        canvas.drawCircle(cx-cellSize*.04f, cy-cellSize*.225f, cellSize*.025f, paint)
        canvas.drawCircle(cx+cellSize*.04f, cy-cellSize*.225f, cellSize*.025f, paint)
        ...
        if (alert) {
            ...
            canvas.drawCircle(cx, cy, cellSize*.31f, paint)
        }
    }
    ...
    private fun drawCustomMarker(canvas: Canvas, rect: RectF, marker: CustomMarker) {
        if (marker == CustomMarker.RAINBOW_GECKO) {
            ...
            colors.forEachIndexed { i, color ->
                paint.color = color
                canvas.drawCircle(rect.centerX(), rect.centerY(), cellSize*(.34f-i*.025f), paint)
            }
            drawGecko(canvas, rect, .85f, false)
            return
        }

        paint.style = Paint.Style.FILL
        paint.textAlign = Paint.Align.CENTER
        paint.textSize = cellSize * .34f
        paint.isFakeBoldText = true
        paint.color = Color.rgb(20, 45, 90)
        val y = rect.top + cellSize*.42f - (paint.ascent()+paint.descent())/2f
        canvas.drawText(marker.symbol, rect.centerX(), y, paint)
        paint.isFakeBoldText = false
    }

    private fun drawGutterHint(canvas: Canvas) {
        paint.style = Paint.Style.FILL
    ...
    private fun cellAt(x: Float, y: Float): Cell? {
        ...
        val col = ((x-boardRect.left)/cellSize).toInt().coerceIn(0, puzzle.size-1)
        val row = ((y-boardRect.top)/cellSize).toInt().coerceIn(0, puzzle.size-1)
        return Cell(row, col)
    }

    private fun cellRect(cell: Cell): RectF {
        val left = boardRect.left + cell.col*cellSize
        val top = boardRect.top + cell.row*cellSize
